...
1. **All quality gates pass locally, with no exceptions** — `./run_tests.sh`, `uv run ruff check kuiskaus/ tests/`, `uv run ty check kuiskaus/ tests/`, `uv run ruff format --check kuiskaus/ tests/`. There is no exemption mechanism and no such thing as a "known-failing" gate: a red gate blocks the merge, full stop.
